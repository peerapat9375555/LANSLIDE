...
## 📝 สิ่งที่อัพเดตล่าสุด 
- **ทำความสะอาดฐานข้อมูล**: ลบตารางเก่าที่ไม่ได้ใช้ออก (`landslide_prediction`, `rainfall_records`, `rainfall_prediction_data`, `landslide_level`, `user_pins`, `user_pinned_locations`, `landslide_events`) และ Constraint ที่เกี่ยวข้อง ลดขนาด SQL จาก 6MB เหลือ ~340KB
- **จัดระเบียบไฟล์**: ย้าย SQL เดิมไป `archive/`, ย้ายสคริปต์ทำความสะอาด (`clean_sql.js`) ไป `database/scripts/`
- **แก้ไข Backend**: ลบการอ้างถึง `user_pinned_locations` ออกจาก `verify_alert` ใน `main.py` ใช้แค่ `user_locations` ในการค้นหาผู้ใช้ใกล้เคียง
...
- ลงดาต้าเบสใหม่ init_database.sql ก่อนใช้งาน (ตาราง: `users`, `user_locations`, `static_nodes`, `rain_grids`, `prediction_logs`, `notifications`, `emergency_services`, `user_reports`)
---
